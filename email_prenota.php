...
"  </tr>".
"  <tr>".
"    <td colspan='2'><span class='style1 style2'>".$_POST['MENSAGEM']."</span></td>".
"  </tr>".
"  <tr>".
"    <td colspan='2'>&nbsp;</td>".
"  </tr>".
...
$Sql = "SELECT PRENFI_IDO, PRENFI_CLASSIFICACAOFISCAL, ".
	...
	while ($RsI = mysql_fetch_assoc($Stmt2)) {
		$Html.="<tr valign='top' bordercolor='#00CCFF' class='style2'>".
			...
			$Html.="<td><div align='center' class='style1'>".$NUMRARS."</div></td>".
				"<td width='11%'><div align='center' class='style1'>".$RsI['PRENFI_CLASSIFICACAOFISCAL']."</div></td>".
				"<td width='6%'><div align='center' class='style1'>".$RsI['PRENFI_UNIDADE']."</div></td>".
				"<td width='6%'><div align='right' class='style1'>".$RsI['PRENFI_QUANTIDADE']."</div></td>".
				"<td width='9%'><div align='right' class='style1'>".formatCurrency($RsI['VALOR_UNITARIO'])."</div></td>".
				"<td width='7%' align='right'><span class='style1'>".formatCurrency($RsI['VALOR'])."</span></td>".
				"<td width='5%' align='center'><span class='style1'>".$Rs['ICMS']."</span></td>".
				"<td width='4%' align='center'><span class='style1'>".$Rs['IPI']."</span></td>".
				"</tr>";
		} 
	$Html.="</table></td>".
		  "</tr>".
		  "<tr>".
		   ...
		   "     <td width='10%' class='style2' ><div align='left' class='style1'><span class='style6'>".formatCurrency($BaseIcms)."</span></div></td>".
		   "     <td width='25%' bgcolor='#FFFF99' a href='cad_defeitos.htm'><div align='right'><strong>Valor do ICMS</strong></div></td>".
		   "     <td width='11%' class='style2' a href='cad_defeitos.htm'><div align='left' class='style1'><strong>".formatCurrency($Icms)."</strong></div></td>".
		   "     <td width='30%' bgcolor='#FFFF99' a href='cad_defeitos.htm'><div align='right'><strong>Valor total dos produtos</strong></div></td>".
		   "     <td width='11%' class='style2' a href='cad_defeitos.htm'><div align='left' class='style1'><strong>".formatCurrency($Rs['BASEICMS'])."</strong></div></td>".
		   "   </tr>";
	  ...
	  "<tr>".
		...
		if ($Rs["SIMPLES"] == "S"){
			...
			$Html.="<table width='100%'  border='0' align='center' class='listagem_procedente'>".
				"<tr>".
	...
	$Html.="</td></tr></table>";
		...
		if (verificar_email($_POST["destinatario"]) == 1){
			$EmailValido = "1";
		}else{
			$EmailValido = "0";
		}
		...
		$men=$_POST['MENSAGEM'];
		$m= new Mail;
		$m->From($_POST["remetente"]);
		$m->To($_POST["destinatario"]);
		$m->ReplyTo($_POST["remetente"]);
		$user=$_SESSION['sId'];
		$dta_log=date("Y-m-d H:i:s", time());
		//$m->Attach($PathImagens. $Rs["PDVRG_FOTO1"], "image/jpeg" );
		$m->Attach($PathImagens. "", "image/jpeg" );
		//$m->Attach($PathImagens. $Rs["PDVRG_FOTO3"], "image/jpeg" );
		$m->Subject("WFA Web :: Multimarca - Envio de Pre-nota " .$_GET["Id"]);
		$m->Body($Html);
		$m->Send(); 
		
		$IDEmail = newIDO();
		$Sql = "INSERT INTO rar_prenf_email (";
		$Sql.= "  PRENFE_IDO, PRENFE_NUMPRENF, PRENFE_USUAR_IDO, PRENFE_DATA, PRENFE_MENSAGEM, PRENFE_DESTINATARIO, PRENFE_REMETENTE, PRENFE_EMAILVALIDO) ";
		$Sql.= "  VALUES (";
		$Sql.= "'".$IDEmail."',";
		$Sql.= "'".$_GET['Id']."',";
		$Sql.= "'".$_SESSION['sId']."',";
