...
$_pagi_sql = "SELECT * FROM rar_transportadoras ";
if (isset($_GET["busca"])){
	$_pagi_sql.= " WHERE transp_nome LIKE '%" . addslashes($_GET["busca"]) . "%'";
}
$_pagi_sql.= " ORDER BY transp_ido";
...
function setSubmit(acao, vlr)
{
	...
	{
		case 'procurar'	:	form.action = 'pesq_transportadoras.php?busca=' + vlr;	form.submit();	break;
		default			:	break;
	}
}
...
<body onLoad="MM_preloadImages('imagens/incluir2.jpg')">
<Form name="form" action="#">
<tr>
    <td height="100%" valign="top" class="tab_conteudo">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="32" class="tab_titulo"><h4>Cadastro de transportadoras</h4></td>
      </tr>
      </table>  
	...
	while($Rs = mysql_fetch_assoc($_pagi_result)) { 
      ...
      <tr bordercolor="<?=$cor?>" class="tab_usuarios_info" onMouseOver="javascript:this.bgColor='#F9F9F9'" onMouseOut="javascript:this.bgColor='#F0F0F0'">
        <td width="22"><input type="checkbox" name="IDS" id="IDS" value="<?=$Rs["transp_ido"]?>" /></td>
        <td><a href="cad_transp.php?Id=<?=urlencode($Rs["transp_ido"])?>"><?=$Rs["transp_nome"]?></a></td>
        <td><?=$Rs["USUAR_LOGIN"]?></td>
        <td align="center"><?=$Rs["transp_contato"]?></td>
        <td align="center"><?=strtolower($Rs["transp_email"])?></td>
		<td align="center">
			<a href="cad_transp.php?Id=<?=$Rs["transp_ido"]?>"><img src="../img/bts/editar.jpg" alt="Editar" width="20" height="20" border="0" /></a>
        	<a href="#" onClick="javascript: deleteById('del_transp.php', '<?=$Rs["transp_ido"]?>');"><img src="../img/bts/apagar.jpg" alt="Excluir" width="20" height="20" border="0" /></a>
		</td>
      </tr>
<? } ?>
	</table>
