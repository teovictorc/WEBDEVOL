...
$_pagi_sql = "SELECT * FROM rar_contato ";
if (isset($_GET["busca"])){
	$_pagi_sql.= " WHERE CONT_NOME LIKE '%" . addslashes($_GET["busca"]) . "%'";
}
$_pagi_sql.= " ORDER BY CONT_NOME";
...
function setSubmit(acao, vlr)
{
	...
	{
		case 'procurar'	:	form.action = 'pesq_contato.php?busca=' + vlr;	form.submit();	break;
		default			:	break;
	}
}
...
<tr>
	...
	while($Rs = mysql_fetch_assoc($_pagi_result)) { 
      ...
      <tr bordercolor="<?=$cor?>" class="tab_usuarios_info" onMouseOver="javascript:this.bgColor='#F9F9F9'" onMouseOut="javascript:this.bgColor='#F0F0F0'">
        <td width="22"><input type="checkbox" name="IDS" id="IDS" value="<?=$Rs["CONT_IDO"]?>" /></td>
        <td><a href="cad_contatos.php?Id=<?=urlencode($Rs["CONT_IDO"])?>"><?=$Rs["CONT_NOME"]?></a></td>
        <td><?=$Rs["CONT_EMAIL"]?></td>
		<td align="center">
			<a href="cad_contatos.php?Id=<?=$Rs["CONT_IDO"]?>"><img src="../img/bts/editar.jpg" alt="Editar" width="20" height="20" border="0" /></a>
        	<a href="#" onClick="javascript: deleteById('del_contatos.php', '<?=$Rs["CONT_IDO"]?>');"><img src="../img/bts/apagar.jpg" alt="Excluir" width="20" height="20" border="0" /></a>
		</td>
      </tr>
<? } ?>
	</table>
