...
function stringToBinary($value) {
	...
	*/
	...
	switch($_GET['cmd']) {
		...
		case "d_c" :
				$Sql = "select NOME, REPLACE(LOGRADOURO, 'É','E') LOGRADOURO, BAIRRO, NM_MUNICIPIO, SG_UF,CONCAT (fantasia,' - ',usuar_nome) AS LANCA_SOLICITANTE
 ...
 WHERE PESSOA = '" .$_GET['PESSOA']. "'" ;
				...
				if ($Rs = mysql_fetch_assoc($Stmt)) {
					...
					if ($Rs2 = mysql_fetch_assoc($Stmt2)) {
						$retorno = "alert(\"" ."Os dados bancários da loja selecionada não estão preenchidos.\\nFavor acessar CADASTROS >> CADASTRO DE CLIENTE e informe os dados.\\n\\nSem os dados preenchimentos corretamente não será possível abrir reclamação!"."\");";
		...
		case "d_s" :  //completa dados de pessoa - WFA Serviços	
				...
				if ($Rs = mysql_fetch_assoc($Stmt)) {
					...
					if ($Rs2 = mysql_fetch_assoc($Stmt2)) {
						...
						if ($_GET['PESSOA'] == 0){  //se for NOVO CLIENTE
							...
							//verifica qual anjo está vinculado ao seu usuário
					...
					$retorno = "alert(\"" ."Atenção, não foram encontrados dados referentes a estrutura de CLIENTE + CONSULTOR + COORDENADOR deste cliente ou o COORDENADOR deste cliente não está vinculado a nenhum anjo.\\nFavor entrar em contato com o Suporte !"."\");";
		...
		case "d_i" :  //completa dados de pessoa - WFA IAF
				...
				if($Rs = mysql_fetch_assoc($Stmt)){
					...
					$retorno = "alert(\"" ."Atenção, já existe questionário respondido para o cliente + pesquisa selecionada  !"."\");";
					...
					if ($Rs = mysql_fetch_assoc($Stmt)) {
						...
						$retorno = "alert(\"" ."Atenção, não foram encontrados dados referentes a estrutura de CLIENTE + CONSULTOR + COORDENADOR deste cliente.\\nFavor entrar em contato com o Suporte !"."\");";
		...
		case "d_WFAPesq" :  //completa dados de pessoa - WFA Pesquisas
				...
				if($Rs = mysql_fetch_assoc($Stmt)){
					...
					$retorno = "alert(\"" ."Atenção, já existe resposta para a pesquisa para o cliente selecionado !"."\");";
		...
		case "d_p" :  //completa dados de pessoa - WFA Serviços	
				...
				//$Sql.= "       , USUAR_IDO, USUAR_NOME ANJO "; //Comentado em 12/05/2008 - IAF não usa Anjo

				$Sql.= " FROM pessoa P, rar_cliente_estrutura RE, pessoa PC"; //Comentado em 12/05/2008 - IAF não usa Anjo ", RAR_ANJO_COORDENADOR, RAR_USUARIO ";
				...
				//$Sql.= "       AND ANJCO_PESSOA = CLIEN_EST_COORDENADOR ";   	//Comentado em 12/05/2008 - IAF não usa Anjo

				//$Sql.= "       AND ANJCO_USUAR_IDO = USUAR_IDO ";				//Comentado em 12/05/2008 - IAF não usa Anjo
				...
				if ($Rs = mysql_fetch_assoc($Stmt)) {
					...
					if ($_GET['Oper'] == "S"){  //verificar se existe operador vinculado
						...
						if ($RsOper = mysql_fetch_assoc($StmtOper)) {
							...
							$OperadorNome = "NÃO VINCULADO";

							$OperadorEmail = "NÃO VINCULADO";
		...
		case "r_1" :  //procura WFA Servicos - inclusao de nota fiscal
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					$retorno = "alert(\"" ."Não foi localizada nenhuma nota fiscal para os dados informados.\\nFavor verificar !"."\");";
		...
		case "r_2" :  //procura WFA Servicos - inclusao de nota fiscal + PRODUTO
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					$retorno = "alert(\"" ."Não foi localizada nenhum produto + nota fiscal para os dados informados.\\nFavor verificar !"."\");";
		...
		case "r_3" :  //procura WFA Servicos - inclusao de nota fiscal de devolução
		...
		case "r_4" :  //procura WFA Servicos - inclusao de duplicata
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					$retorno = "alert(\"" ."Não foi localizada nenhuma duplicata para os dados informados.\\nFavor verificar !"."\");";
		...
		case "r_5" :  //procura WFA Servicos - inclusao de pedido de venda
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					$retorno = "alert(\"" ."Não foi localizado nenhum pedido de venda para os dados informados.\\nFavor verificar !"."\");";
		...
		case "r_7" :  //procura WFA Servicos - inclusao de pedido de venda + produto
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					$retorno = "alert(\"" ."Não foi localizado nenhum pedido de venda + produto para os dados informados.\\nFavor verificar !"."\");";
		...
		case "r_9" :  //procura WFA Servicos - inclusao de nota fiscal + PRODUTO
			...
			$Sql = " SELECT * ";
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					$retorno = "alert(\"" ."Não foi localizado produto informado.\\nFavor verificar !"."\");";
		...
		case "r_11" :  //procura WFA Servicos - inclusao de nota fiscal + PRODUTO + n.º par
		...
		case "r_13" :  //procura WFA Servicos - inclusao de solicitacao de material
			...
			if ($_POST['SERSM_IDO'] == ""){
				...
				if (!$Rs = mysql_fetch_assoc($Stmt)) {
					...
					//$retorno = "alert(\"" ."Referência de produto não localizada no banco de dados.\\nFavor verificar !"."\");";
		...
		case "iaf_pa_inc" :  //inclusao de plano de ação - iaf
		...
		case "iaf_paacomp_inc" :  //inclusao de acompanhamento - plano de ação - iaf
		...
		case "remover" :  //Remove WFA Servicos
			...
			if ($_GET['Tipo'] == 14){
				...
				//procura ultimo registro de acompanhamento para o plano de ação em questão
		...
		case "p_r" :
				...
				$Sql =  "select r.grupo_material, P.NOME, nofi.PESSOA_EMITENTE,nofi.NUM_NF,nofi.SERIE_NF,".
						...
						" group by r.grupo_material, P.NOME, nofi.PESSOA_EMITENTE, nofi.NUM_NF,nofi.SERIE_NF,".
				        "       DT_EMIS_NF ". 
						"	    ,itnf.CD_ITEM_MATERIAL ". 
						"       ,itnf.CD_COLECAO ". 
						"       ,itnf.LANCAMENTO ". 
				...
				if ($Rs = mysql_fetch_assoc($Stmt)) { 
					if (!$Rs = mysql_fetch_assoc($Stmt)) {
						...
						if ($Rs["DIAS"]  > 9999 && $_GET['PESSOA'] != "18500"){
							...
							if ($Rs["grupo_material"]  != $_GET['Categoria']){
								$retorno.= "alert(\"" ."A categoria selecionada não pertence a referência informada !"."\");";
							}else{
					...
					if ($Rs = mysql_fetch_assoc($Stmt)) {  
						...
						$retorno = "alert('A nota fiscal informada não foi localizada no banco e dados !');";				
		...
		case "p_f" :
         ...
         " where   itpv.cd_item_material = '".$_GET['REFERENCIA']. "' ";
			...
			if ($x > 1){
				...
				$retorno.= "alert('Atenção!!! Esta referência foi produzida por mais um fabricante. Favor verificar o código do fabricante na sola do calçado.');";
