<?php	include("cache.inc.php");	
	...
	mysql_select_db("dbWEBDEVOL",$Conn);
	$NomeDesenvolvedor = "";	
	$FoneDesenvolvedor = "";	
	$CelDesenvolvedor  = "";	
	...
	// um e-mail v�lido, preciso configurar o mail	
	//$PathImagens = "C:/Inetpub/wwwroot/andarella_mysql/fotos/"; 
	//Precisa de permiss�o de Escrita e Leitura	
	$PathImagens = "C:/wamp/www/webdevol/fotos/";
	//Precisa de permiss�o de Escrita e Leitura	
	$PathImportacao = "C:/wamp/www/webdevol/importacao/";
	//Precisa de permiss�o de Escrita e Leitura	
	...
	function PrevisaoConclusao($Servi_numero) {
		...
		$Data = "Diferen�a: ".$Rs["DIFERENCA_DIAS"]." - Abertura: ".$Rs["servi_dataabertura"] ." - Prazo: " .$Rs["prazo"] ." - Previsao: " .$Rs["PREVISAO"];
	...
	if ($Rs = mysql_fetch_assoc($Stmt)){
		...
		if ($Rs["USUAR_EFRANQUEADO"] == "S"){
			...
			if ($Rs["USUAR_CONSU_PESSOA"] == ""){
				...
				//se for coordenador pode ver apropria��es cadastradas pelo usu�rio e por usu�rios cuja o c�digo de pessoas seja igual um de seus consultores 
				...
				//se for consultor, ent�o somente as apropria��es cadastradas pelo usu�rio poder�o ser vistas
		...
		//echo("Categoria usu�rio = ".$CategoriaUsuario. " - ConsuPessoa = ".$ConsuPessoa);
	...
	if ($IAFCliente == true){
		...
		if ($IAFPQ_IDO != ""){
			...
			//Seleciona quais categorias v�o entrar no IAF
	...
	function AtualizaStatusIAF($QUEST_IDO) {
		...
		if ($Rs = mysql_fetch_assoc($Stmt)){
			...
			if ($Rs["IAFPQ_PLANEJAMENTO"] == 0){  //se IAF nao solicitar planejamento j� encerra PLANEJAMENTO
			...
			if ($Rs["IAFPQ_PLANOACAO"] == 0){  //se IAF nao solicitar planejamento j� encerra PLANEJAMENTO

				$QUEST_STATUS_PA = 1;

			}else{

				//se necessitar plano de a��o, verifica se tem algum �tem que necessita plano de a��o
