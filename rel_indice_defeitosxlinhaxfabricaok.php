<? $Title = "Índice de Defeitos x Fábrica x Linha";
	...
	$SQL = "select F.NOME, sum(I.item_qtde) as QTDE ";
...
<? $Total = 0;
	...
	while($Rs = mysql_fetch_assoc($Stmt)){ ?>
	  ...
	  <tr>
			...
			<td colspan="4" bgcolor="#f5f5f5" class="imp_normal_total_fundoamarelo"><strong>F&aacute;brica:&nbsp;</strong><?=$Rs["PESSOA"]?> - 
		...
		while($Rs2 = mysql_fetch_assoc($Stmt2)){ ?>
	 		...
	 		<tr>
			  ...
			  <td colspan="3" bgcolor="#D6F0FE" class="imp_normal_total"><strong>Linha: <?=$Rs2["LINHA"]?></strong></td>

			  <td bgcolor="#D6F0FE" class="imp_normal_total"><strong>Quantidade:&nbsp;</strong><?=number_format($Rs2["QTDE"], 0, ',', '.')?></td>

			  <td bgcolor="#D6F0FE" class="imp_normal_total" align="right"><?=round(($Rs2["QTDE"]/$Rs["QTDE"]) * 100,2)?>% sobre f&aacute;brica </td>
			...
			$SQL.= "       and substr(ITEM_referencia,1,4) = '" .$Rs2["LINHA"]. "' ";
			...
			while($Rs3 = mysql_fetch_assoc($Stmt3)){ ?>
				...
				<tr>
					...
					<td bgcolor="#f5f5f5" class="imp_normal_total"><div align="right"><?=round(($Rs3["QTDE"]/$Rs2["QTDE"]) * 100,2)?>&nbsp;% sobre linha </div></td>
