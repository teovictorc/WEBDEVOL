<? $Title = "Índice Geral Mensal - Modelo";
...
<?  
	...
	while($Rs = mysql_fetch_assoc($Stmt)){
		...
		while($Rs2 = mysql_fetch_assoc($Stmt2)){
			...
			?>
                ...
                <tr class="imp_normal">
                  ...
                  <td bgcolor="#f5f5f5"><?=$Rs2["linha"]?></td>

                  <td align="right" bgcolor="#f5f5f5"><?=number_format($Rs2["PAR_RECLAMADO"], 0, ',', '.')?></td>

                  <td align="right" bgcolor="#f5f5f5"><?=number_format($Rs2["VLR_RECLAMADO"], 2, ',', '.')?></td>

                  <td align="center" bgcolor="#f5f5f5"><?=number_format($Perc, 2, ',', '.')?>                    % </td>
				...
				<tr class="imp_normal">
				  ...
				  <table width="95%"  border="0" align="right">
                    ...
                    <tr>
                      ...
                      <td width="33%" class="imp_normal_total"><strong>Modelo</strong></td>

                      <td width="22%" class="imp_normal_total"><div align="center"><strong>

                        <?=$UnidadeMedida?>

                      </strong></div></td>

                      <td width="22%" class="imp_normal_total"><div align="center"><strong>Valores</strong></div></td>

                      <td width="22%" class="imp_normal_total" align="center"><div ><strong>% sobre 
		...
		$SQL = "select substr(item_referencia,5,4) modelo, ";
		...
		while($Rs3 = mysql_fetch_assoc($Stmt3)){
			...
			$ValorP+= $Rs3["VLR_RECLAMADO"];

			$QtdeP+= ($Rs3["PAR_RECLAMADO"]);

			$ValorPT+= ($Rs3["VLR_RECLAMADO"]);

			$QtdePT+= ($Rs3["PAR_RECLAMADO"]);

			$Perc = round(($Rs3["PAR_RECLAMADO"]/$Rs2["PAR_RECLAMADO"])*100,2);
			...
			?>
                    ...
                    <tr class="imp_normal">
                      ...
                      <td align="right"><?=number_format($Rs3["PAR_RECLAMADO"], 0, ',', '.')?></td>

                      <td align="right"><?=number_format($Rs3["VLR_RECLAMADO"], 2, ',', '.')?></td>

                      <td align="center"><?=number_format($Perc, 2, ',', '.')?>
