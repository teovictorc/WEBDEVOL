<? $Title = "Índice de Defeitos x Linha x Modelo";
...
<? $Total = 0;
	...
	while($Rs = mysql_fetch_assoc($Stmt)){ ?>
		...
		<tr>
			...
			<td colspan="4" bgcolor="#f5f5f5" class="imp_normal_total_fundoamarelo"><strong>Linha:&nbsp;</strong><?=$Rs["LINHA"]?></td>

			<td bgcolor="#f5f5f5" class="imp_normal_total_fundoamarelo"><strong>Quantidade:&nbsp;</strong><?=number_format($Rs["QTDE"], 0, ',', '.')?></td>

		    <td width="16%" bgcolor="#f5f5f5" class="imp_normal_total_fundoamarelo"><div align="right"><?=round(($Rs["QTDE"]/$TotalDefeitos) * 100,2)?>&nbsp;% sobre total </div></td>

		 </tr>

		 <?

		$SQL = "select substr(ITEM_referencia,5,4) as MODELO, sum(I.item_qtde) as QTDE ";
		...
		while($Rs2 = mysql_fetch_assoc($Stmt2)){ ?>
	 		...
	 		<tr>
			  ...
			  <td colspan="3" bgcolor="#D6F0FE" class="imp_normal_total"><strong>Modelo: <?=$Rs2["MODELO"]?></strong></td>

			  <td bgcolor="#D6F0FE" class="imp_normal_total"><strong>Quantidade:&nbsp;</strong><?=number_format($Rs2["QTDE"], 0, ',', '.')?></td>

			  <td bgcolor="#D6F0FE" class="imp_normal_total" align="right"><?=round(($Rs2["QTDE"]/$Rs["QTDE"]) * 100,2)?>% sobre linha </td>
			...
			$SQL.= "       and substr(ITEM_referencia,5,4) = '" .$Rs2["MODELO"]. "' ";
			...
			while($Rs3 = mysql_fetch_assoc($Stmt3)){ ?>
				...
				<tr>
					...
					<td colspan="2" bgcolor="#f5f5f5" class="imp_normal_total">Grupo de defeito:&nbsp;<?=$Rs3["D_NOME"]?></td>

					<td width="17%" bgcolor="#f5f5f5" class="imp_normal_total">Quantidade:&nbsp;<?=number_format($Rs3["QTDE"], 0, ',', '.')?></td>

					<td bgcolor="#f5f5f5" class="imp_normal_total"><div align="right"><?=round(($Rs3["QTDE"]/$Rs2["QTDE"]) * 100,2)?>&nbsp;% sobre modelo </div></td>
				...
				$SQL.= "       and A.AVALI_AREZ_DEFEIG_IDO = '" .$Rs3["DEFEIG_IDO"]. "' ";
				...
				while($Rs4 = mysql_fetch_assoc($Stmt4)){?>
					...
					<tr>
					  ...
					  <td width="48%" class="imp_normal">Sub-grupo de defeito:&nbsp;<?=$Rs4["D_NOME"]?></td>

					  <td class="imp_normal">Quantidade:&nbsp;<?=number_format($Rs4["QTDE"], 0, ',', '.')?></td>

					  <td class="imp_normal"><div align="right"><?=round(($Rs4["QTDE"]/$Rs3["QTDE"]) * 100,2)?>&nbsp;% sobre o grupo </div></td>
  ...
  <tr>
    ...
    <td colspan="4" class="imp_normal"><div align="right"></div>      

	<div align="right"><strong>Total:</strong></div></td>

    <td colspan="2" class="imp_normal"><div align="left">&nbsp;&nbsp;<?=number_format($TotalDefeitos, 0, ',', '.')?></div></td>
