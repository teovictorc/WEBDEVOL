...
$_pagi_sql = " SELECT * FROM rar_defeito_grupo ";
if (isset($_GET["busca"])){
	$_pagi_sql.= " WHERE DEFEIG_DESCRICAO LIKE '%" . addslashes($_GET["busca"]) . "%'";
}
$_pagi_sql.= " ORDER BY DEFEIG_CATEGORIA, DEFEIG_DESCRICAO ";
...
function setSubmit(acao, vlr)
{
	...
	{
		case 'procurar'	:	form.action = 'pesq_defeitos_grupo.php?busca=' + vlr;	form.submit();	break;
		default			:	break;
	}
}
//-->
...
<tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="32" class="tab_titulo"><h4>Cadastro de grupo de defeito</h4></td>
      </tr>
      </table>  
	...
	while($Rs = mysql_fetch_assoc($_pagi_result)) { 
      ...
      <tr bordercolor="<?=$cor?>" class="tab_usuarios_info" onMouseOver="javascript:this.bgColor='#F9F9F9'" onMouseOut="javascript:this.bgColor='#F0F0F0'">
        <td width="22"><input type="checkbox" name="IDS" id="IDS" value="<?=$Rs["DEFEIG_IDO"]?>" /></td>
        <td><a href="cad_defeitos_grupo.php?Id=<?=$Rs["DEFEIG_IDO"]?>"><?=$Categoria?></a></td>
        <td><?=$Rs["DEFEIG_DESCRICAO"]?></td>
		<td>
			<div align="center"><a href="cad_defeitos_grupo.php?Id=<?=$Rs["DEFEIG_IDO"]?>"><img src="../img/bts/editar.jpg" alt="Editar" width="20" height="20" border="0" /></a>
		    <a href="#" onClick="javascript: deleteById('del_defeitos_grupo.php', '<?=$Rs["DEFEIG_IDO"]?>');"><img src="../img/bts/apagar.jpg" alt="Excluir" width="20" height="20" border="0" /></a>	          </div></td>
      </tr>
<? } ?>
	</table>
	<br/ >
