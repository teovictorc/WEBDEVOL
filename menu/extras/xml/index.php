	...
	for($a=1;$a<count($xmlA);$a++)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					$old = replaceInnerText($old, "\"", "\"", "=", "testeEQUALS", 1);
					$old = replaceInnerText($old, "\"", "\"", " ", "testeSPACE", 1);
					...
					for($b=1;$b<count($splitProp);$b++)
					{
						...
						$prop[1]=ereg_replace("testeEQUALS","=",$prop[1]);
						$prop[1]=ereg_replace("testeSPACE"," ",$prop[1]);
...
<SCRIPT language=\"JavaScript\" src=\"$pathToCodeFiles$file_milonicsrc\" type=\"text/javascript\"></SCRIPT>	
